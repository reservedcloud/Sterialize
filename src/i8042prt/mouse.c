...
{
	i8042MouWait( ); IoOutputByte( 0x64, 0xD4 );
	i8042MouWait( ); IoOutputByte( 0x60, ucValue );
}
...
)
{
	...
	{
		case 0:
			if ( i8042MouPacketReady ) break;
			if ( ( ucData & 0b00001000 ) == 0 ) break;
...
)
{
	...
	if ( i8042MouPacket[ 0 ] & 0b00010000 ) xNegative = 1;
	else xNegative = 0;

	if ( i8042MouPacket[ 0 ] & 0b00100000 ) yNegative = 1;
	else yNegative = 0;

	if ( i8042MouPacket[ 0 ] & 0b01000000 ) xOverflow = 1;
	else xOverflow = 0;

	if ( i8042MouPacket[ 0 ] & 0b10000000 ) yOverflow = 1;
	else yOverflow = 0;

	if ( !xNegative )
	{
		KiSystemCursor.X += i8042MouPacket[ 1 ];
		if ( xOverflow ) KiSystemCursor.X += 255;
	}
	else
	{
	...
	if ( !yNegative )
	{
		...
		if ( yOverflow ) KiSystemCursor.Y -= 255;
	}
	else
	{
	...
	if ( KiSystemCursor.X < 0 ) KiSystemCursor.X = 0;
	if ( KiSystemCursor.X > VidScreenWidth - 1 ) KiSystemCursor.X = VidScreenWidth - 1;

	if ( KiSystemCursor.Y < 0 ) KiSystemCursor.Y = 0;
	if ( KiSystemCursor.Y > VidScreenHeight - 1 ) KiSystemCursor.Y = VidScreenHeight - 1;
